...
export function Application({ application, onChange }: { application: VendorApplication; onChange: () => void }) {
  ...
  return <article className="panel"><div className="order-heading"><div><p className="eyebrow">{application.category}</p><h2>{application.businessName}</h2></div><span className="badge">{application.status}</span></div><dl className="details-grid"><div><dt>Legal entity</dt><dd>{application.legalEntityName}</dd></div><div><dt>Edible Shop registration number</dt><dd>{application.registrationNumber}</dd></div>{application.legacyRegistrationNumber && <div><dt>Previously supplied business number</dt><dd>{application.legacyRegistrationNumber}</dd></div>}<div><dt>Location</dt><dd>{application.location}</dd></div><div><dt>Contact</dt><dd>{application.contactPerson?.name}<br />{application.contactPerson?.email}<br />{application.phone || application.contactPerson?.phone}</dd></div></dl>{application.description && <p className="preserve-lines">{application.description}</p>}<Feedback error={action.error} success={action.success} />{application.status === 'Pending' ? <form className="form-stack" onSubmit={submit}><label>Review notes<textarea name="adminNotes" rows={3} maxLength={2000} /></label><div className="action-row"><button className="button primary" value="Approved" disabled={action.busy}>Approve seller</button><button className="button secondary danger" value="Rejected" disabled={action.busy}>Reject application</button></div></form> : application.adminNotes && <p className="notice">{application.adminNotes}</p>}</article>;
}
